...
md"""
# MTG 3D reconstruction

This notebook is used to reconstruct a plant in 3D from measurements in an MTG file. 
"""

# ╔═╡ 651c27b1-bbc2-4973-960b-efed6af012c3
md"""
...
Read the MTG file, and automatically update when the file changes:
"""
...
"""
	cylinder(node::MultiScaleTreeGraph.Node, parent_plane)

Build a cylinder using the node attributes and the upper-plane of the cylinder of the parent.

The attributes must be:

- `azimuthal_angle`: the azimuthal angle in degree
- `zenithal_angle`: the zenithal angle in degree, i.e. the elevation
- `length`: the node length in cm
- `diameter`: the node diameter in mm
"""
function cylinder(node::MultiScaleTreeGraph.Node, parent_plane)
    ...
    n = Meshes.Vec3(x, y, z) # vector normal to the plane at point p1
    p1 = parent_plane.p + n * node[:length]
    return Meshes.Cylinder(node[:diameter] / 10.0 / 2.0, parent_plane, Meshes.Plane(p1, n))
end

# ╔═╡ 92dff6b8-e631-4248-9248-b16d6abc71d0
"""
	build_wood!(node::MultiScaleTreeGraph.Node, parent_plane=Meshes.Plane(Meshes.Point3(0.0, 0.0, 0.0), Meshes.Vec3(0.0, 0.0, 1.0)))

Build the cylinders of an MTG, for the nodes called "Segment" and "Phytomer".
"""
function build_wood!(
    node::MultiScaleTreeGraph.Node,
    parent_plane=Meshes.Plane(Meshes.Point3(0.0, 0.0, 0.0), Meshes.Vec3(0.0, 0.0, 1.0))
)
...
begin
	filename = "mtg_test_geometry.xlsx"
	update_time = PlutoLinks.@use_file_change(filename)
	mtg = @use_memo([update_time]) do
		read_mtg(filename)
	end
	branching_order!(mtg)
	build_wood!(mtg)
...
git-tree-sha1 = "f5e8663954224e05d42a6ace40b1beffade7d5dc"
